...
public:
    ...
    void Generate(PT pt);
    void Generate_GPU(PT pt);
    // 将优先队列最前面的一个PT
    void PopNext();
    int total_guesses = 0;
    vector<string> guesses;

    void BatchGenerate(const vector<PT>& pts); // 声明批量生成函数
    void submit_gpu_batch_async(const vector<PT>& pts);
    void check_and_collect_gpu_results();
    void wait_for_any_gpu_task();
};